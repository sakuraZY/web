      ...
      <div class="flex-content">
        <div class="flex-main">
          ...
          <el-row v-if="arrflowstep.includes(1)" v-show="stepNum === 1" key="1">
            <el-col :span="21">
              <dydj ref="children" @btnDisabled='btnDisabled' @funcNext="handleNext"></dydj>
            </el-col>
            <el-col :span="3">
          ...
          <query-success v-if="arrflowstep.includes(3)" v-show="stepNum === 3" ref="children"
            :infoJson="infoJson" key="3"></query-success>
        </div>
      </div>
    </template>
    <!-- 底部暂存和提交栏 -->
        ...
        <div class="footflex" v-if="stepNum !== 3">
          <div class="foot-btn">
            <ul class="btnList">
              ...
              <li v-if="stepNum !== 2">
                <el-button type="primary" @click="handlerStorage"
                :disabled="isDisabled">暂存</el-button>
              </li>
              <li>
                <el-button type="primary" @click="handleNext"
                v-if="stepNum === 1" :disabled="isDisabled">下一步</el-button>
                <el-button type="primary" @click="handleNext"
                v-if="stepNum === 2">提交</el-button>
              </li>
            </ul>
          </div>
        </div>
...
import dydj from '@/components/realReg/dydj';
import { mapState, mapMutations } from 'vuex';
import QueryUpload from '@/components/queryUpload';
import QuerySuccess from '@/components/querySuccess';
// import { saveDYDJ, submitUrlDYDJ } from '@/apis/realReg/ygdy';
import { submitUrlDYDJ } from '@/apis/nres/ygdy';
...
export default {
  ...
  data() {
    return {
      ...
      indexs: 0,
      infoJson: {
        messJson: ['dydj', 'MESSAGEJSON'],
        nodeName: ['dydj', 'NODENAME'],
        submitUrl: submitUrlDYDJ,
        parentPnode: 'dydj',
      },
      isDisabled: true,
    };
  ...
  computed: {
    ...mapState('dydj', { arrflowstep: state => state.ARRFLOWSTEP }),
  },
  methods: {
    ...mapMutations('dydj', {
      GO_NEXT: 'GO_NEXT',
    ...
    btnDisabled(bdisabled) {
      ...
      if (bdisabled) {
        ...
        this.menulist = ['不动产数据检验', '抵押人信息', '抵押权人信息', '不动产权信息', '第三方债务人信息', '抵押信息'];
      }
    },
  },
  mounted() {
