<template>
  <div class="form-message-box" v-show="isVisable">
    <div id="qlrDiv" ref="dyrDiv" class="tableDiv">
      ...
      <div class="info-body">
         <personTable  :personData='qlrDataFormat' :headNames='headNamesQlr'
      :showBtn='!showBtn' :showObj='showObjQlr' @addFamilyFn='addFamily'></personTable>
      </div>
    </div>
      ...
      this.qlrData.map((item) => {
        if (item.name === this.selectQlr.name) {
          item.family = this.$refs.familyTable.familyData.map((tmpitem) => {
            tmpitem.zjlb = this.tranZJLBId(tmpitem.zjlb, this.zjlxlist);
            return tmpitem;
          });
        }
        return item;
