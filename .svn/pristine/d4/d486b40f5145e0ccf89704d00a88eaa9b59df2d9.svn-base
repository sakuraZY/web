 ...
 * @LastEditTime: 2020-03-20 17:36:05
-->
<template>
...
export default {
  ...
  methods: {
    ...mapMutations('queryData', [
      'SET_MYTEMPFLOWCOUNT',
      'SET_MYEXTRAFLOWCOUNT',
      'SET_MYFINISHEDFLOWCOUNT',
      'SET_QUERYSQBH',
    ]),
    ...
    getFlowPath(ywlxbm = '', type = 'update') {
      if (type === 'update') {
        return this.flowList.submitSetting.filter(p => p.ywlxbm === ywlxbm);
      }
      return this.flowList.detailSetting.filter(p => p.type === 'view');
    },
    clearPageInfo() {
    ...
    getFlowList() {
      const { submitSetting, detailSetting } = flowConfig;
      this.flowList = { submitSetting, detailSetting };
    },
  },
  mounted() {
    this.handleQuery();
